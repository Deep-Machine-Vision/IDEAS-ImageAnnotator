...
class SaveTool(object):
    ...
    def prepare_directory(self, file_name):
        dir_path = os.path.dirname(file_name)
        if not os.path.exists(dir_path):
            os.makedirs(dir_path, exist_ok=True)

    def overlay_rangeMask_onRGB(self, rgbI, maskI, rgb_alpha=0.4, mask_alpha=0.6):
        cmap    = plt.get_cmap('jet')
        new_img = np.delete(cmap(maskI), 3, 2) * 255
        new_img[maskI<1e-3, :] = 0
        return np.clip(rgbI * rgb_alpha + new_img * mask_alpha, 0, 255).astype(np.uint8)
    ...
    def save_group_pilImage_RGB(self, images,
        ...
        '''
        self.prepare_directory(save_path)
    ...
    def save_single_pilImage_gray(self, image,
        ...
        @Param: image -- image in size [ht, wd]
                ...
                autoScale -- if palette is 'range', scale the value in image to 0~255.
        '''
        self.prepare_directory(save_path)
        if resize is not None:
            if isinstance(resize, list):
...
class OverlayDetectionBGR(object):
    ...
    def overlay_boxes(self, bgrI, boxes, classes=None, random_color=False, ignores=[]):
        '''
        ...
        for k, (box, color) in enumerate(zip(boxes, colors)):
            if box is None or k in ignores:
                continue
    ...
    def overlay_mask(self, bgrI, masks, colors=None, classes=None, show_mask=True,
                           random_color=False, img_ratio=0.6, mask_ratio=0.6, ignores=[]):
        '''
        @Param: bgrI -- [ht, wd, 3] in BGR mode
        ...
        visited = None
        for k, (mask, color) in enumerate(zip(masks, colors)):
    ...
    def overlay_class_names_capbox(self, bgrI, classes, boxes, scores=None, ignores=[]):
        '''
        @Param: bgrI -- [ht, wd, 3] in BGR mode
                boxes -- list ob bbox for [x0,y0,x1,y1]
                classes -- list of category id
        ...
        ht, wd      = bgrI.shape[:2]
        for k, (box, score, label, color) in enumerate(zip(boxes, scores, class_names, colors)):
            if box is None or k in ignores:
                continue
            x, y     = box[:2]
            ...
            cap_x0 = x          if x + text_w < wd else x - text_w
            cap_x1 = x + text_w if x + text_w < wd else x
            cap_y0 = y - text_h if y - text_h > 0  else y
            cap_y1 = y          if y - text_h > 0  else y + text_h
            ...
            cv2.putText(bgrI, text_str, (cap_x0, cap_y1),
                        font_face, font_scale, text_color, font_thickness, cv2.LINE_AA)

        return bgrI

    def overlay_detection_onBGR(self, bgrI, boxes=None, masks=None, classes=None, scores=None, mask_colors=None):
        '''
        ...
        if masks is not None:
            bgrI, ignores = self.overlay_mask(bgrI, masks, mask_colors)
        if boxes is not None:
            bgrI = self.overlay_boxes(bgrI, boxes, classes, ignores=ignores)
        if boxes is not None and classes is not None:
            bgrI = self.overlay_class_names_capbox(bgrI, classes, boxes, scores=scores, ignores=ignores)
        return bgrI
