...
def unary_from_labels(labels, n_labels, gt_prob, zero_unsure=True, adapt_weight=False):
    """
    ...
    if adapt_weight:
        label_bins = list(np.unique(labels))
        label_bins.append(label_bins[-1]+1)

        label_hist,_ = np.histogram(labels, bins=label_bins)
        label_w0   = 1/np.sqrt(label_hist)
        label_w1   = label_w0 / np.min(label_w0)
    
        # label from 'labels' to k would add energy 'n_energy * weight'
        label_w  = np.zeros(np.max(label_bins))
        label_w[label_bins[:-1]] = label_w1
        for k in range(n_labels):
            U[k,:] = n_energy * label_w[labels]
